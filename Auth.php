...
define('CLIENT_SECRET', '8jiF1A3x1ZA9QWG7mHdcbVXEvh8');
...
class Auth {
   ...
   public static function getTokenFromAuthCode($code, $state) {
      ...
      $result = file_get_contents('https://www.reddit.com/api/v1/access_token', false, $context);
      return $result;
   }
   ...
   public static function setUserToken($token, $refreshToken, $expires_in) {
      ...
      $query = <<<EOT
         ...
         `token_expires` = UNIX_TIMESTAMP() + (:expires_in)
         WHERE `userName` = :user
EOT;
   ...
   public static function getToken($user) {
      ...
      $query = <<<EOT
         SELECT `redditToken`, `redditRefreshToken`, `token_expires`
         FROM `users`
         WHERE `userName` = :user
EOT;

      $statement = $db->prepare($query);
      $statement->bindParam(':param', $user);
      list($token, $refreshToken, $expires) = $statement->execute();
...
EOT;
      $statement = $db->prepare($query);
      $statement->bindParam(':user', $user);
      $refreshToken = $statement->execute();
...
EOT;
      ...
      list($expires_in, $list) = $statement->execute();
